...
app.post('/login', async (c) => {
    try {
        ...
        const [rows] = await pool.query("SELECT id,room,email,password,role FROM users WHERE email=? LIMIT 1", [email]);
        const user = rows[0];
        if (!user) {
...
app.post('/register', async (c) => {
    try {
        ...
        await pool.query("INSERT INTO users (room,email,password,role) VALUES (?,?,?,?)", [room, email, password, "user"]);
...
app.get('/users', async (c) => {
    try {
        const [rows] = await pool.query("SELECT id,room,email,role FROM users ORDER BY room ASC");
        return c.json(rows);
    }
    catch {
        return c.json({ message: "Server error" }, 500);
...
WHERE id=?`, [room, email, password, role, id]);
...
app.post('/machines/add', async (c) => {
    try {
        ...
        const [exist] = await pool.query("SELECT id FROM machines WHERE machine_number=?", [machine_number]);
        if (exist.length > 0) {
            return c.json({ message: "Machine number already exists" }, 400);
        }
        await pool.query("INSERT INTO machines (machine_number,status,current_user_name) VALUES (?,?,?)", [machine_number, "available", null]);
...
`);
        ...
        for (const machine of machines) {
            ...
            if (now >= end) {
                await pool.query(`UPDATE machines
